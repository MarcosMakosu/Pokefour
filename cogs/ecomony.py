...
class Economy(commands.Cog):
    ...
    async def use(self, ctx, *, args: str):
        ...
        effect = __import__("json").loads(effect_json)
        # Localiza o Pokémon
        try:
            poke_id = int(target)
        ...
        if effect_type == "heal":
            ...
            max_hp = calc_hp(poke[6], poke[5], poke[4])  # indices: level=4, iv_hp=5, base_hp precisa ser carregado
            # Simplificação: usaremos current_hp do banco (índice 13)
            current = poke[13]
            if hp_restore == "full":
                new_hp = max_hp
            else:
            ...
            # Lógica de evolução por pedra – requer integração com cadeia de evolução
